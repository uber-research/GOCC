...
// AddToEdgeDivWithType increases the weight of an edge between two nodes. If
// there isn't such an edge one is created.
func (n *Node) AddToEdgeDivWithType(to *Node, dv, v int64, residual, inline bool, edgeType EdgeType) {
	if n.Out[to] != to.In[n] {
		panic(fmt.Errorf("asymmetric edges %v %v", *n, *to))
	}

	if e := n.Out[to]; e != nil {
		if n.Out[to].Type != edgeType || to.In[n].Type != edgeType {
			panic(fmt.Errorf("asymmetric edge types %v %v", *n, *to))
		}
	...
	info := &Edge{Src: n, Dest: to, WeightDiv: dv, Weight: v, Residual: residual, Inline: inline, Type: edgeType}
	n.Out[to] = info
	to.In[n] = info
}
...
func (n *Node) AddToEdgeDiv(to *Node, dv, v int64, residual, inline bool) {
	n.AddToEdgeDivWithType(to, dv, v, residual, inline, Call)
}
...
const (
	Call EdgeType = iota
	GoInvocation
	ChannelSendToChannelReceive
	ChannelSendToSelect
	SelectToChannelReceive
	SelectToSelect
	MaybeTimeoutToSelect
	Unknown
)
